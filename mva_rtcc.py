...
class MvaRtcc:
    ...
    def evt_btnCreate_clicked(self):
        ...
        myRendererMva = QgsCategorizedSymbolRenderer("Surface Elev (ft)", myRenderListMva)
        ...
        for feature in layerRtccTemp.getFeatures():
            ...
            buffergeom = geom.buffer(3*1852,90) # 3NM buffer with 90 vertices per quarter turn
        ...
        features = []
        field_index = layerRtccTemp.fields().indexOf('Surface Elev (ft)')
        # Retrieve all features and store them along with their height values
        for feature in layerRtccTemp.getFeatures():
            features.append((feature, feature['Surface Elev (ft)']))
        # Sort features by "Surface Elev (ft)" in descending order
        ...
        myRendererRtcc = QgsCategorizedSymbolRenderer("Surface Elev (ft)", myRenderListRtcc)
